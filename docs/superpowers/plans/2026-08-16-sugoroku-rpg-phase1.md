...
function renderPlayers(state) {
  ...
  for (const player of state.players) {
    ...
    // player.name comes from free-text input in main.js (Task 9), so it must
    // go through textContent, never innerHTML — otherwise a player-chosen
    // name containing markup would be parsed as HTML.
...
`index.html`を静的サーバー経由で開き(例: `npx serve .`、`python3 -m http.server`など)、以下を確認する。ES Modulesは`file://`直接読み込みだとCORSでブロックされるため、ブラウザで直接ファイルを開く方法は使わないこと:
